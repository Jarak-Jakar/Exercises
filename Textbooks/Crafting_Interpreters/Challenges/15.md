...
I'm not really sure how, at this point, we are supposed to measure a performance improvement within Lox.  The `clock()` function is mentioned right in the opening of Chapter 14, but I don't believe any form of implementation is given for it.  It would seem to me that any attempt to measure performance differences would need to involve using something external to Lox, which isn't a *big* deal (especially not when things like the excellent [Hyperfine](https://github.com/sharkdp/hyperfine) exist), but I also kinda figure that without even looping constructs yet in the language, there's not really a decent way to do any form of performance measurement.

As for which instructions could benefit from similar optimisations?  It would seem to me that any operation where the height of the stack ends up unchanged *and* only involves the one operand could benefit from this.  Of the instructions that have been implemented in Clox so far, I don't think any of the others fit those criteria.  Presumably the vast majority of unary operators would qualify, though.  For example, logical negation, rather than numerical negation.  Also, potentially any time a new value is assigned to the 'variable' at the top of the stack, this could be applied.  I suspect, however, that in those instances you could probably simply remove one or more of the previous operations, since the result of them is probably never used (which is, of course, one of the classic optimisations of any good compiler).

Taking a quick squiz at the Lox grammar in the book's appendix, the only operations I can see which would suit this optimisation are indeed logical and numerical negation, though I imagine it is quite possible that I have overlooked/forgotten something else.
