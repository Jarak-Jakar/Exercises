...
let inputFilePath = Path.Combine([| Path.GetDirectoryName(Util.CurrentQueryPath); "inputs"; "input_5.txt" |])
...
for step in steps do
    ...
    // The only difference from 5a is that I don't use List.rev on top on the below line
    stacks[step.dst] <- List.append top stacks[step.dst]
